#this code is base on Google tensorflow tutorial
...
def main(unused_argv):
  ...
  mnist_classifier = tf.estimator.Estimator(
      model_fn=cnn_model_fn, model_dir="/final_mnist_hopefully/mnist_convnet_model")
  ...
  model_dir = "/final_mnist_hopefully/mnist_convnet_model" 
  path = os.path.join(model_dir,str(int(time.time())))

  def serving_input_receiver_fn():
      feature_tensor = {"x": tf.placeholder(shape=[None, 784], dtype=tf.float32)}
